...
def evaluate_experiment(y_true, y_pred, thresholds=None):
    ...
    # tmp_list.append(f1_score(y_true[:, i], y_pred[:, i]))

    # results['macro_f1'] = np.mean(tmp_list)
...
def load_raw_data_ptbxl(df, sampling_rate, path):
    if sampling_rate == 100:
        ...
        if os.path.exists(path + 'raw500.npy'):
            ...
            data = np.array([signal for signal, meta in data])
            # data.dump(path+'raw500.npy')
            with open(path + 'raw500.npy', 'wb') as fin:
                pickle.dump(data, fin, protocol=4)
...
def ICBEBE_table(selection=None, folder='../output/'):
    ...
    for model in models:
        ...
        for col in cols:
            ...
            unc = max(me_res.loc['upper'][col] - me_res.loc['point'][col],
                      me_res.loc['point'][col] - me_res.loc['lower'][col])
...
def random_crop(X, fs=100, crops=1):
    final_data = []
    if fs == 100:
        length = int(2.5 * fs)
        for data_idx in range(X.shape[0]):
            for i in range(crops):
                start_idx = np.random.randint(0, X[data_idx].shape[0] - length)
                tmp_data = X[data_idx][start_idx:start_idx + length]
                Sfinal_data.append(tmp_data)
    else:
        pass
    return np.array(final_data)


def remark_label(Y, crops):
    newy = []
...
def slide_and_cut(tx, ty, window_size, stride):
    final_data = []
    final_pid = []
    final_label = []
    tmpid = 0
    ...
    for idx in range(tx.shape[0]):
        ...
        n_length = tx[idx].shape[0]

        i = 0
        for i in range(0, n_length - window_size + 1, stride):
            final_data.append(tx[idx][i:i + window_size])
            final_pid.append(tmpid)
            final_label.append(ty[idx])

        if i + stride + window_size - n_length < 0.5 * stride:
            final_data.append(tx[idx][-window_size:])
            final_pid.append(tmpid)
            final_label.append(ty[idx])

        tmpid += 1

    return np.array(final_data), np.array(final_label), np.array(final_pid)
